...
from .fixtures import (backend, caldav, principal, journal_fixtures)


@pytest.mark.asyncio
async def test_create_journal_1(caldav, principal, journal_fixtures):
...
@pytest.mark.asyncio
async def test_create_journal_2(caldav, principal, journal_fixtures):
    cal_id = uuid.uuid4().hex
    cal = await principal.make_calendar(name="Yep", cal_id=cal_id,
...
@pytest.mark.asyncio
async def test_create_delete_calendar_with_journal(caldav, principal, journal_fixtures):
    cal_id = uuid.uuid4().hex
    cal = await principal.make_calendar(name="Yep", cal_id=cal_id,
                                        supported_calendar_component_set=['VJOURNAL'])
...
@pytest.mark.asyncio
async def test_create_journal_from_vobject(caldav, principal, journal_fixtures):
    cal_id = uuid.uuid4().hex
    cal = await principal.make_calendar(name="Yep", cal_id=cal_id)
...
@pytest.mark.asyncio
async def test_lookup_journal_1(caldav, principal, journal_fixtures):
    cal_id = uuid.uuid4().hex
    cal = await principal.make_calendar(name="Yep", cal_id=cal_id)
...
@pytest.mark.asyncio
async def test_delete_journal_1(caldav, principal, journal_fixtures):
    cal_id = uuid.uuid4().hex
    cal = await principal.make_calendar(name="Yep", cal_id=cal_id)
